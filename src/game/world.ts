...
export function createWorld(scene: Scene): World {
  scene.background = new Color('#86d8ff')
  scene.fog = new Fog('#86d8ff', 22, 105)
  ...
  for (let i = 0; i < pos.count; i++) {
    ...
    const creekZ = -18 + Math.sin((v.x + 10) * 0.06) * 5
  ...
  // Procedural sky dome (cheap PSO-ish)
  ...
  npc.position.set(2.5, heightAt(2.5, -1.5), -1.5)
  root.add(npc)

  // Portal
  const portal = makePortal()
  portal.position.set(-3.8, heightAt(-3.8, -3.2), -3.2)
  root.add(portal)
  ...
  return {
    root,
    spawnPoint: new Vector3(0, heightAt(0, 4), 4),
    npcs: [{ id: 'guide', name: 'Town Guide', object: npc }],
    portals: [{ id: 'hedge-instance', name: 'Hedge Mission', object: portal }],
    collectibles,
    heightAt,
  }
}
